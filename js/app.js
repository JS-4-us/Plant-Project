...
function InDoorPlant(name, discreption) {
  this.name = name;
...
function OutDoorPlant(name, discreption) {
  this.name = name;
  this.discreption = discreption;
...
function creatingIndoorObjects(inDoorPlants, Discreption) {
  for (let i = 0; i < inDoorPlants.length; i++) {
    var temp = new InDoorPlant(inDoorPlants[i], Discreption[i]);
    arrayOfPlants.push(temp);
  }
}

function creatingOutdoorObjects(outDoorPlants, Discreption) {
  for (let i = 0; i < outDoorPlants.length; i++) {
    var temp = new OutDoorPlant(outDoorPlants[i], Discreption[i]);
    arrayOfPlants.push(temp);
  }
}
...
creatingIndoorObjects(inDoor, inDoorDiscreption);
creatingOutdoorObjects(outDoor, outDoorDiscreption);
